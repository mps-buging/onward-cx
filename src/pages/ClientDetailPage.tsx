...
import { ArrowLeft, ArrowUp, Check, ChevronDown, ChevronRight } from "lucide-react"
        ...
        <div className="border-t border-border pb-2 pt-1">
          {milestone.subtasks.length === 0 ? (
            ...
            milestone.subtasks.map((subtask) => {
              const isSubtaskNoteOpen =
                noteFor?.type === "subtask" && noteFor.id === subtask.id
              ...
              return (
                <div key={subtask.id}>
                  <div className="group flex items-center gap-2 px-3 py-1.5">
                    ...
                    <InlineEdit
                      value={subtask.title}
                      onSave={(t) => onSaveSubtaskTitle(milestone.id, subtask.id, t)}
                      className={cn(
                        "flex-1 text-sm",
                    ...
                    <button
                      type="button"
                      onClick={() => onNoteTarget("subtask", subtask.id)}
                      className={cn(
...
export function ClientDetailPage() {
  ...
  if (loading) {
    return (
      <PageShell client={null} progress={0} onSaveClientName={saveClientName}>
  ...
  return (
    <PageShell client={client} progress={progress} onSaveClientName={saveClientName}>
